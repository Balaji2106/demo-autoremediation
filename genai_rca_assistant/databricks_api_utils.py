...
def extract_error_message(run_data: Dict) -> Optional[str]:
    ...
    for task in tasks:
        ...
        if result_state == "FAILED":
            ...
            if not real_error:
                ...
                if real_error:
                    logger.info(f"   ⚠️  Only found generic state_message for task '{task_key}'")
            ...
            if real_error:
                ...
                logger.warning(f"   ⚠️  Task '{task_key}' failed but no error message found in any field!")
    ...
    if not error_messages:
        ...
        if job_error:
            ...
            logger.warning("   ⚠️  No job-level error found either!")
